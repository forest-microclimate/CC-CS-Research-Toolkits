...
## When to invoke
Building any model that fuses ML with a mechanism for K67 flux / interior-profile work — PINN, gray-box, or emulator. Trigger on:
...
1. **Gray-box / hybrid (DEFAULT here).** Conservation + transport hard-coded; ML parameterizes ONLY the genuinely-uncertain closure — K(z,t), soil/canopy respiration, stomatal response. Mechanism where physics is known, ML where it isn't. The collaborator's ANN-beats-MEP shape, generalized. Budget closes by construction.
...
- Hold out a REGIME, not random rows: the 2015-16 ENSO drought (Oct 2015 - Mar 2016; gap fraction 43-52% vs ~5% baseline). A physically-consistent model extrapolates there; a black-box overfits the training climate.
...
- This is exactly why the collaborator's ANN beats MEP MOST clearly in the drought.
...
- A constrained conditional mean still loses 43-78% of diel variance (the metric trap) and can be miscalibrated (K67 height cov95 ran 0.52-0.77 = under-covered).
