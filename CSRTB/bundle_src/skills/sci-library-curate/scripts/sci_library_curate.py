...
import os as _os_shim, sys as _sys_shim
_sys_shim.path.insert(0, _os_shim.path.dirname(_os_shim.path.abspath(__file__)))
import sci_lib_common as _c
_JSTOP        = _c._JSTOP
_asciify      = _c.asciify
_family_name  = _c.family_name
_surname_sep  = _c.surname_sep
_name_features = _c.name_features
_camel        = _c._camel
norm_title    = _c.norm_title
real_doi      = _c.real_doi
journal_abbrev = _c.journal_abbrev
canonical_stem = _c.canonical_stem
blocking_keys = _c.blocking_keys
write_atomic   = _c.write_atomic
format_author  = _c.format_author
abbreviate_authors = _c.abbreviate_authors
...
# ---- I16-I19 supplement-linkage & cryptic-name invariant primitives (defects: orphan/cryptic/mislink) ----
# Shared by cmd_validate (I16-I19) and link_supplements (content-DOI gate). Deterministic; no runtime
# agent judgment. DOI comparison reuses common.real_doi (imported as real_doi above) so an SI's own DOI
# is compared to its parent's under the SAME real-DOI gate the rest of the module uses.
...
# supplement/version suffixes an SI DOI carries relative to its parent article DOI
...
# Data-repository DOI prefixes. An SI that carries its OWN repository DOI (Dryad, Zenodo, figshare,
# FLUXNET, PANGAEA, EDI, Dataverse, ...) legitimately differs from its parent's article DOI — the
# link_supplements docstring protects exactly these. So a repo DOI on the SI side is NEVER an I18
# disagreement and NEVER rejects a link; it is skipped (relies on the parent_file linkage instead).
_REPO_DOI_PREFIXES = (
    "10.5061/",   # Dryad
    "10.5281/",   # Zenodo
    "10.6084/",   # figshare
...
# Author_Year clean_name: a Capitalized author token (letters/diacritic/hyphen/apostrophe) then _YYYY.
...
def link_content_doi_gate_rejects(si_doi, parent_doi):
    """link_supplements DOI-cluster fallback content gate (defense-in-depth behind I18): REJECT a link
    when BOTH sides carry a real, non-repo DOI and they disagree after suffix-normalization. A repo DOI
    on the SI side never rejects (a Dryad/Zenodo/figshare SI legitimately differs from its parent)."""
    if _is_repo_doi(si_doi):
        return False
    a, b = _norm_doi_cmp(si_doi), _norm_doi_cmp(parent_doi)
    if not a or not b:
        return False
    return a != b
    ...
    for s in supps:
        ...
        if a is None:                                      # (2) shared-key fallback
            ...
            if cand:
                ...
                # CONTENT-DOI GATE (defense-in-depth behind validator I18): even if a broadcast DOI
                # clustered this SI with `cand`, REJECT the link when the SI's OWN stored DOI and the
                # candidate parent's stored DOI both exist and DISAGREE after suffix-normalization. A
                # repo DOI on the SI side never rejects (a Dryad/Zenodo/figshare SI legitimately
                # differs from its parent's article DOI). The parent_file/primary path (1) is unaffected.
                doi_conflict = link_content_doi_gate_rejects(s.get("doi", ""), arts.get(cand, {}).get("doi", ""))
                if not (a_sn and s_sn and a_sn != s_sn) and not doi_conflict:
                    a = cand
        if a is not None:
...
def cmd_validate(args):
    ...
    I16 no unresolved supplement: every supplement/dataset resolves to a MAIN parent OR carries orphan_parent_absent (FAIL)
    I17 no cryptic clean_name: no article/supplement keeps a publisher-code/bare stem without cryptic_unresolved (FAIL)
    I18 SI/parent DOI agreement: a linked SI's own content DOI (--lib) or stored DOI must match its parent's (FAIL)
    I19 article identity real: an article row must have a non-blank title (FAIL)
    """
    idx = _read_index(args.index)
    ...
    if args.lib:
        ...
        _LIT_EXT = (".pdf", ".txt", ".docx", ".xlsx", ".xls", ".doc",
                    ...
                    ".r", ".rmd", ".ipynb", ".html", ".htm",
                    ".fasta", ".fa", ".nex", ".nwk", ".tre", ".tsv")   # defect #60: analysis-code/rendered-data/sequence SI formats
        for dp, _, fs in os.walk(args.lib):
            for f in fs:
    ...
    # ==== I16-I19: supplement-linkage & cryptic-name invariants (close the false-green blind spots) ====
    # These four cover the failure classes I1-I15 structurally could not see: orphan/mislinked SIs,
    # cryptic clean_names shipped silently, cross-paper SI mislinks, and titleless articles. Deterministic.
    _by_name = {}
    for r in idx:
