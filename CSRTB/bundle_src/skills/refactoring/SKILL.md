---
name: refactoring
description: The structural-change procedure — invoke WHEN a change dissolves or replaces a core abstraction (a dichotomy, data structure, coordinate/"currency", aggregation level, shared assumption), WHEN a subsystem has accumulated ~3+ patches to the same area (the recurring-patch-saga tell), or WHEN tempted to add a mirror-knob or bracket-search a threshold. Owns the re-derive-the-invariants checkpoint, the old-currency grep, and unify-over-parallel-patch. The HOW of the refactor-invariants discipline (re-derive the full invariant set when an abstraction dissolves). NOT the build discipline (→ software-craft) and NOT symptom debugging (→ CODE_REVIEW_DEBUGGER profile + root-before-bandaid discipline).
---
<!-- SPDX-License-Identifier: MIT · Copyright (c) 2026 Neill Prohaska <forest.microclimate@gmail.com> -->
# STATUS: CURRENT (2026-07-28). Authored in the software-developer pass. Operationalizes the refactor-invariants discipline as a runnable procedure (the always-on TELLS live in the standing disciplines; this owns the steps).
...
REF: `software-craft` (baseline-before-extend, the same move at smaller scale) · `testing-discipline` (the checks that re-encode the NEW invariants) · root-before-bandaid (the sibling discipline on the symptom side: diagnose the root before suppressing the symptom).
