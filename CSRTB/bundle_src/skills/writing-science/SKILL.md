...
description: >
  Diagnose and revise science prose using Joshua Schimel's Writing Science framework
  (OCAR, story structures, the funnel, topic/stress positions, given-to-new flow) paired
  with a mechanical detector kernel that flags draft-level tells (nominalizations, passive
  voice, weak/fuzzy verbs, empty amplifiers, buried verbs, weak gap framing, undermining
  resolutions, rhetorical flourishes / "cheap cleverness," fancy words, wordy phrases,
  expletive openers, naked demonstratives, and more). Load when reviewing or writing a manuscript, abstract, proposal,
  cover letter, or any scientific document, or when the user asks whether prose is clear,
  strong, sticky, or flabby, or when about to return prose you drafted yourself — the
  nine-tic self-scan. The kernel measures the draft's actual shape before any
  judgment; the frameworks handle the structure the kernel cannot see.
---
...
2. **The detector kernel** (`kernel.py`, auto-loaded) — sentence/word-level tells a script surfaces reliably: nominalization+light-verb, agentless passive, fuzzy verbs, empty amplifiers, buried main verbs, "little is known" gap framing, "more research is needed" endings, undefined acronyms, and — added from a full mining of Schimel plus Strunk & White — **rhetorical flourish** ("cheap cleverness"), fancy words, wordy phrases, expletive openers, naked demonstratives, negations that should be positive, misused words, and oversized paragraphs; plus profile stats (sentence-length distribution, verb/word ratio, nominalization & noun-train density, em-dash/semicolon/exclamation counts).

The split is load-bearing: an LLM agent reliably FORGETS the mechanical tells and reliably HALLUCINATES having checked them ("the prose is clean and direct") ⇒ the mechanical layer is a script you RUN, not a habit you recall. The script cannot judge whether a paragraph earns its place ⇒ that stays with you. **Run the kernel to see the draft's real shape; use the frameworks to decide what the shape means.**

## The first move, always: measure before you judge

WHEN about to characterize a draft in any way — call it "clear," "dense," "strong," "wordy" — ⇒ run the kernel and read the profile FIRST. Stating a verdict about text without measuring it is the single tell this skill exists to prevent (the ported "theory-before-measurement" failure). The detectors are candidate-flaggers: they surface suspects reliably; **you** decide whether each flagged item is actually a defect here, for this reader. Nothing in the kernel auto-edits.

```python
# kernel.py is auto-loaded when this skill loads. If running standalone:
import kernel as ws
print(ws.report(draft_text))          # triage: profile + tell tally + top hits per tell
r = ws.scan_draft(draft_text)         # structured: r["counts"], r["profile"], r["hits"]
over = ws.find_overcorrection(before_text, after_text)  # compare mode: hedges/field-standard terms a de-jargon pass may have wrongly stripped (BWT-007)
```

Read `r["profile"]` first (sentence-length distribution, verb/word ratio ~0.15 is
comfortable, nominalization and noun-train density per 100 words, em-dash/semicolon/
exclamation counts). Then triage `r["counts"]`, and only then open the specific
`r["hits"][name]` lists to disposition individual sentences. A count of 90 "repeated_words"
is a place to look, not 90 edits to make.

### The same rule applies to your OWN output, not just the user's draft

The failure this skill most often leaks is not missing a tell in someone else's text — it
is **writing one yourself and not noticing**, because when you compose a sentence you are
attending to the meaning, not the mechanics, and you reliably believe your own prose is
clean. The rule is therefore symmetric: **prose you WRITE or REVISE gets the same kernel
scan as prose you review, before you deliver it.** WHEN you are about to hand back a rewritten
paragraph, a suggested sentence, an abstract you drafted ⇒ run `ws.scan_draft()` on YOUR
text and disposition the hits, exactly as you would the user's. This is the composition-time
check that a review-only habit skips.

The concrete case that motivated the flourish detectors: an agent revising a
manuscript wrote *"each of these efforts stops at organisms a camera, a net, or a rain
collector can capture"* — a rhetorical triplet chosen for effect — recognized the anti-
flourish rule only after the user pushed back, and had never scanned its own sentence. The
`flourish_triad` detector now fires on that construction; but the detector only helps if you **run it on what
you write**, which is why this is stated as a TELL, not an exhortation to "write plainly."
...
kernel's detectors live at the Clarity/Flow/Language tiers; Structure is yours alone.
...
quick — if you take more than two steps, you will stumble"* (p.44). The kernel flags the
...
paragraphs. The kernel's `trailing_qualifier`, `citation_position`, and `buried_verbs` tells
...
the self-scan the per-profile self-scan clause mandates: **run the detectors on your OWN
draft, then work this checklist.** These nine tics (BWT-001..009) were catalogued from
Claude-authored science prose; each entry gives the **primary cue** (what fires it), the
**plain test**, and the **fix that preserves the exact claim**. Every flag is a candidate you
dispose — recast a real hit *without letting the plain version assert more or less than the
evidence supports.*
...
4. **overclaim / overstatement (BWT-004)** — *cue:* absolute or superlative phrasing
   ...
   whether the design supports the absolute goes to the Formal & Quantitative Argument
   Checker.
...
**Detector-layer note:** the detector-documentation sections of this skill and the auto-loaded
`kernel.py` carry the post-P2 split detector engine (the flourish split, `novelty_claims`,
`term_drift`, `pseudo_explanation`, `em_dashes`, the `absolute_quantifiers` arms, and the
`find_overcorrection` compare callable) — see `### New & augmented detectors` and
`## The detector kernel` below. The items in this section are the JUDGMENT-TIER passes and
named checks the two-round test produced — they live in your reading pass, independent of the
detector layer.
...
POST-REFRESH STATUS: the post-P2 split engine (`kernel.py`) BUILT three of these — `term_drift`
...
in `scan_draft()` output drops the `find_` prefix.)
...
- `absolute_quantifiers` **(BWT-004, LOW severity)** — the strength lexicon {barely, never, always,
  ...
  **resultative/verb-form absolutes** ("avoids", "eliminates", "guarantees", "never fails") in an
  UNHEDGED claim. Evidence-fit stays redirected to the Formal & Quantitative Argument Checker.
...
- **BWT-007 has NO per-draft detector** — it is a reasoning tic in the de-jargon pass. **P2 adds a
  COMPARISON callable** `ws.find_overcorrection(before, after)` that surfaces hedges and
...
## The detector kernel — what it flags and how to read it
...
Detectors marked `approximate=True` (noun trains, buried verbs, verb ratio, the flourish split (`flourish_triad`/`flourish_epigram`/`flourish_metaphor`/`apologetic_contrast`), `code_metaphor_leakage`, `figurative_mannerism`, `pseudo_explanation`, `term_drift`, `misused_words`, `naked_this`, `pseudo_suffix`, `scare_quotes`) use
POS-free heuristics — treat them as directional candidates the reader disposes. The kernel strips markdown scaffolding (code
...
the book concept accurately and by page where you can; invent no rules. When the kernel and
...
This skill is prose craft. Argument validity and logical structure belong to the AI-Safety
Scholar; formal or quantitative claims to the Formal & Quantitative Argument Checker;
machine-facing document/prompt design to the LLM Doc Architect. The reader you are writing
for is a human.
