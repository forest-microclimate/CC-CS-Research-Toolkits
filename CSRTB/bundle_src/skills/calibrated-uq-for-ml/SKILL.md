...
- **Quantile regression forests** — fit the quantile loss at τ=0.025 & 0.975 directly; natural first choice here (trees beat the bam 45/46 folds). Caveat: quantiles saturate at the training range → they do NOT widen out-of-support, so weak for T0.
...
2. **PIT histogram** (needs a full predictive sample, not just endpoints). `pit_values(y_true, samples)`, `samples` shape [n_obs, n_draws] → per-obs PIT ∈ [0,1]. Calibrated ⇒ PIT ~ Uniform(0,1). The shape names the defect:
   - ∪-shaped (mass piled at 0 & 1) = UNDER-dispersed / overconfident — the K67 failure.
...
Split-conformal guarantees MARGINAL coverage and assumes cal & test are exchangeable. The drought transfer fold (Oct 2015 – Mar 2016) is a nonstationary regime — cal blocks from wet years are NOT exchangeable with it.
