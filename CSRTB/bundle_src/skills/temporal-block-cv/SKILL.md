...
## Related
preflight-parallel (fold fan-out); mgcv-temporal-gam (the fitted model); brms-hierarchical-fitting (the Bayesian gold stage); assert the per-fold event count; model-check by calibration/coverage when there is no ground-truth "true value".
