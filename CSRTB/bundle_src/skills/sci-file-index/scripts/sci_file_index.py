...
def cmd_build(args):
    ...
    # DOI-BROADCAST GUARD (defect #61): NEVER overwrite an SI's OWN mined doi with the parent's,
    # and if the SI carries its own doi that DISAGREES with the parent's (after stripping a
    # -supplement/-suppl*/.s0* suffix), the link is suspect (a broadcast/citation-bleed doi) — keep
    # the SI's real doi and stamp a note so validator I18 can catch the cross-paper mislink rather
    # than the disagreement being silently masked by inheritance.
    def _norm_doi(d):
        d = (d or "").strip().lower()
    ...
    for r in rows:
        if r["record_type"] == "supplement" and r["parent_file"] in byname:
            ...
            if own_doi and _norm_doi(own_doi) != _norm_doi(p.get("doi","")) and _norm_doi(p.get("doi","")):
    ...
    # CRYPTIC-NAME FLAG (defect #61): when identity resolution leaves a row with no real author
    # (blank/Unknown) on an article/book/etc., its canonical clean_name cannot be built and would
    # ship as a raw publisher/DOI code. Stamp `cryptic_unresolved` so validator I17 has its explicit
    # escape flag and the completeness report surfaces it — instead of shipping a cryptic name silently.
    for r in rows:
