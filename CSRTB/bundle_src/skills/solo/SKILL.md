...
DEF.dial: solo is the AUTONOMY-MAX detent of a single three-level AGENCY dial held in ONE frame-memory variable `AGENCY_MODE ∈ {solo, collaborative, plan}` (/solo = autonomy MAX · `/collab` = collaborative MIDDLE default · `/plan` = deliberation MAX). One variable, one value ⇒ the levels are mutually exclusive BY CONSTRUCTION; the same row is read/written by skills plan, collab, and this skill.
DEF.state_substrate: `AGENCY_MODE` persists in FRAME memory — `write_memory(entity="frame", ...)`. Frame memory survives context compaction within THIS conversation but is per-session ⇒ it matches the dial's "state is per-conversation" semantics. Each new conversation starts at collaborative, so /solo must be re-fired per conversation.
RULE.set_mode: WHEN `/solo` fires ⇒ in ONE write, set the frame-memory dial row to `AGENCY_MODE: solo (since <ts>)` (replace the single dial row; append only if absent). This one assignment simultaneously leaves collaborative and plan — there is no second flag to clear. WHEN unsure mid-conversation ⇒ read the frame dial row to recover the active level before acting.
RULE.visible_marker: WHILE `AGENCY_MODE=solo` ⇒ lead each response with `— solo mode on —` so the user always sees the active level; drop it when the dial moves off solo. (/plan shows `— plan mode on —`; /collab shows no marker — it is the neutral default.)
NOTE.vs_recon_floor: the always-on commit-recon discipline (survey + self-adjudicate before a COMMIT-CLASS action) fires in EVERY mode, solo included — the pre-commit survey still runs. Solo does NOT disable it; it only means the SELF-adjudicated go/no-go is trusted without routing to the user (whereas /plan routes that same go/no-go TO the user).
...
- WHEN autonomously executing a multi-track PLAN (sub-agents in flight, results landing) ⇒ actively DIRECT it: supervise each wave, and re-route or adapt the plan as results warrant rather than mechanically running a stale plan to completion. This is deciding-for-yourself applied to orchestration — the HOW (the supervise→decide→act loop) lives in the `directing-execution` skill; the routing choices in `delegation-planning`. (Applies when you are the orchestrating lead — e.g. under the PLANNER profile; a single-track domain task does not become a cascade just because the dial is on solo.)
...
- (b) a FATAL, specifically-NAMED blocker in {code | convergence | time | data | access} you cannot resolve.
...
## Ending the mandate
The mandate holds while `AGENCY_MODE=solo`; it ends when the user moves the dial — fires `/plan` (⇒ deliberation max) or `/collab`, or utters any RETURN-TO-MIDDLE phrase (the canonical set owned by skill collab: "solo off / go ahead / back to normal / step by step / check with me / …" ⇒ `AGENCY_MODE=collaborative`), or gives a superseding directive. The dial does NOT persist to the next conversation; each starts at `collaborative`.

# REF: skill plan (the deliberation-MAX opposite detent) · skill collab (the middle default, owns DEF.return_phrases) · the always-on commit-recon floor (fires in every mode). AGENCY_MODE lives in frame memory (write_memory entity="frame").
INVARIANT: solo trusts your self-adjudicated go/no-go without a user gate — but the commit-recon survey still runs, and the two RULE.pause reasons still stop for the user.
