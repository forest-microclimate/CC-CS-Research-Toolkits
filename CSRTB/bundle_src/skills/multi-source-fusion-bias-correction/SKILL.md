...
- **Validate by transfer to a held-out regime, not in-sample fit.** Hold out an overlap window from a regime unlike the mean state — the Oct 2015–Mar 2016 ENSO drought (gap fraction rises to ~52% CO₂ / 43% H₂O vs ~5% baseline) — learn the correction on the rest, check it transfers there. Blocked, not random-iid folds (`temporal-block-cv`).
- **Emit provenance + uncertainty.** Tag each filled timestamp with its source and correction method; fusion reclassifies a was-NA large gap into a source-tagged fill — coordinate the canonical T0–T3 tier labels with the tower pipeline (`MICROMET_RECONSTRUCTOR` / `gap-fill-imputation`). The corrected-source residual over the overlap IS the empirical error model for that source in the gap — hand it to `calibrated-uq-for-ml` for calibrated intervals (bias correction fixes the mean/shape; it does not by itself calibrate spread).

## Kernel functions (auto-loaded with this skill)
...
All three learn on the co-indexed overlap pair and apply to the full source. Proven route: the collaborator's 2000–2023 SW reconstruction fused tower `Raw` + `MSG` (Meteosat) + `SYN1DEG` (CERES) + `ERA5-Single`, ML-harmonized to the tower (Fig 6), same treatment for LW, net radiation, Tair, humidity, wind, pressure, precipitation.
