...
2. AUTHOR `payload/settings/<name>.fragment.json` registering it under the right hook EVENT with a `matcher` and `command` using a `$HOME` path (NOT an absolute `/Users/...` path - it must resolve on any machine):
```json
...
## Claude Science analog (do not conflate the two surfaces)
This skill is the CLAUDE CODE side (files under `~/.claude/`, `install.sh`). The CLAUDE SCIENCE analog of "add an extension" is `host.skills.*` / `host.agents.*` via the `customize` skill (published skills, agent profiles) - no install.sh, no fragments; the artifact/lineage system replaces backup/versioning. WHEN porting a toolkit extension onto Claude Science => choose the native primitive (skill, profile, kernel.py sidecar, delegation) rather than transliterating the hook/fragment mechanism. Refs: `customize`, `skill-creator`, `bash-hook-contract`.
