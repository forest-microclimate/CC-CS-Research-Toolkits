...
## Related
temporal-block-cv + cluster-bootstrap (evaluate + infer); gap-fill-imputation (chunk+splice when predicting to fill); validate the right column/units before trusting a term; state the evidence for any effect claim.
