...
RECEIPT_TOKEN_PATTERNS = {
    "verified":       r"\bverified\b",
    "passed":         r"\bpassed\b",
    "byte-identical": r"\bbyte[\s_\u2014\u2013-]?identical\b",
    "all-green":      r"\ball[-\s]+green\b",
    "all-N-passed":   r"\ball[\s_-]+\d+[\s_-]+passed\b",
    "addressed":      r"\baddressed\b",
    "reproduced":     r"\breproduced\b",
    "confirmed":      r"\bconfirmed\b",
}
RECEIPT_TOKEN_RES_CACHE = {}

def receipt_token_res():
    """Lazily compile RECEIPT_TOKEN_PATTERNS (kept literal at module top so the sidecar
    gate accepts the file). Cached module-level; case-insensitive."""
    import re as re_
    if not RECEIPT_TOKEN_RES_CACHE:
        for k_, p_ in RECEIPT_TOKEN_PATTERNS.items():
            RECEIPT_TOKEN_RES_CACHE[k_] = re_.compile(p_, re_.I)
    return RECEIPT_TOKEN_RES_CACHE
VALID_RECEIPT_KINDS = {"hash", "diff", "exit_code", "read_ref", "test_tally"}

# require_verification_status -- DISC-18 efficacy-from-existence.
VALID_VSTATUS = {"verified-working", "attempted-untested", "unknown"}
VSTATUS_REQUIRE_CITATION = {"verified-working"}
...
def require_receipt(claim_text, receipts):
    ...
    return {"gate": "require_receipt", "verdict": verdict, "tokens": found,
            ...
            "marker": marker_("receipt_gate", verdict, tokens=len(found), receipts=valid)}
...
def require_verification_status(row):
    ...
    if not isinstance(row, dict):
        ...
        return {"gate": "require_verification_status", "verdict": "FAIL",
                ...
                "marker": marker_("vstatus_gate", "FAIL")}
    ...
    elif status in VSTATUS_REQUIRE_CITATION and blank_(row.get("citation")):
    ...
    return {"gate": "require_verification_status", "verdict": verdict,
            ...
            "marker": marker_("vstatus_gate", verdict)}


# ─── CLI (the ONLY place that prints or exits) ───────────────────────────────
def read_input_(path):
