...
description: Close your own loop instead of asserting a state you have not checked. Invoke WHEN about to write a durable claim about a state you did not just recompute — a count ("48 skills"), a field value ("sha is abc123"), or a state ("the gate passes") — into a memory row, artifact, report, changelog, or handoff. Also invoke WHEN authoring a NEW check and you need the recipe for turning a manual check into a mechanically-fired loop. Ships verify_claims(), a kernel assertion that RAISES on any claim-vs-record mismatch and, fail-closed, on a vacuous zero-claim check (override with allow_empty=True), emitting a [[vloop:...]] marker so an auditor can tell "checked and clean" from "never ran". Use it BEFORE the claim ships. Also ships require_receipt() and require_verification_status(): return-based sibling gates (CLI, exit 0/2) for a "verified"/"passed" claim with no receipt (DISC-07), or a "this fix works" claim with no honest verification_status (DISC-18).
---
...
7. **Keep the kernel publish-safe (CS sidecars).** A CS sidecar kernel's module top level
   may hold ONLY plain-name function defs, imports, and literal-constant assigns to plain
   names — no computed values (`re.compile`/`frozenset`/any `Call`), no `_`-prefixed names
   (defs or assigns), no top-level `if`/`for`/`with` incl. the `__main__` guard. Verify with
   `python3 check_sidecar_contract.py` before any build.
