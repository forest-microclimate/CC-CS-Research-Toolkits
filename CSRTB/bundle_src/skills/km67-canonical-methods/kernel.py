...
"""km67 canonical-methods registry + self-verifying lineage check.

Loaded by the `km67-canonical-methods` skill. Provides:
  km67_registry()        -> the full registry dict
  km67_whatis(var)       -> canonical engine + product for one variable (verified facts)
  km67_verify(var=None)  -> RE-DERIVE the engine mapping from LIVE host.lineage and
                            diff against the registry; the anti-drift self-check.
...
SHIPPED PRODUCT's lineage actually ran -- never a standalone engine file's docstring.
"""
import re
...
# architecture-marker probes used by km67_verify to RE-DERIVE arch from live lineage code
ARCH_MARKERS = {
...
def km67_whatis(var):
    ...
    These are verified facts; run km67_verify(var) to re-confirm against live lineage."""
    r = REGISTRY.get(var)
    if r is None:
...
def km67_classify_arch(code):
    """Re-derive architecture markers present in a block of lineage code."""
    found = set()
    low = code.lower()
...
def km67_verify(var=None, verbose=True):
    """ANTI-DRIFT SELF-CHECK. For each variable, read the SHIPPED PRODUCT's live
    lineage (host.lineage[product_version]) and re-derive the architecture markers,
    then flag any that contradict the registry (e.g. a 'global_bam_only' signature
    where the registry says chunked composite). Returns a dict of per-var results.
    Requires the `host` singleton (available in python/r kernels)."""
    try:
        host  # noqa: F821
    except NameError:
        print("km67_verify needs the `host` singleton (run in a python/r kernel).")
        return None
    targets = [var] if var else list(REGISTRY)
    out = {}
    for v in targets:
        r = REGISTRY.get(v)
        if r is None:
            out[v] = {"status": "UNKNOWN_VAR"}; continue
        try:
            code = (host.lineage[r["product_version"]].get("code") or "")  # noqa: F821
        except Exception as e:
            out[v] = {"status": "LINEAGE_ERROR", "error": str(e)}; continue
        arch = km67_classify_arch(code)
        rfiles = sorted(set(re.findall(r'/(?:v[0-9a-f]{8}_)?([A-Za-z0-9_]+\.R)', code)))
        # the one hard contradiction: shipped product showing the tombstoned global-bam-only signature
        drift = "global_bam_only" in arch
        out[v] = {
            "status": "DRIFT" if drift else "ok",
            "derived_arch": sorted(arch),
            "R_files_in_lineage": rfiles,
            "expected_engine": r["engine_file"],
        }
        if verbose:
            flag = "  <-- DRIFT: shipped product shows global-bam-only!" if drift else ""
            print("%-10s %-6s arch=%s%s" % (v, out[v]["status"], sorted(arch), flag))
    return out
