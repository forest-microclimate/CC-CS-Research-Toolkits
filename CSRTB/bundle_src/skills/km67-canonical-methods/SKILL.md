...
The kernel sidecar auto-loads these functions in your python/R kernel:

1. `km67_whatis("co2")` — the verified engine + product + architecture for one variable.
2. `km67_verify()` — **the anti-drift self-check.** Re-reads each shipped product's LIVE
   `host.lineage[...]`, re-derives the architecture markers, and flags any contradiction
   (e.g. a product that shows a global-bam-only signature where the registry says chunked
   composite). Run it whenever the registry might be stale, before trusting it in a brief,
   or after any re-ship. `status: "ok"` = registry matches reality; `"DRIFT"` = investigate.
3. `km67_registry()` / `km67_donotuse()` — full registry / tombstoned artifacts.

If `km67_verify()` returns DRIFT for any variable, STOP and reconcile before proceeding —
the registry has fallen behind the shipped artifacts.
...
When a variable's engine or shipped product is re-shipped, update `kernel.py`'s `REGISTRY`
(new product/engine version ids) in the same session, then run `km67_verify()` to confirm
the registry matches the new lineage. The registry stores ids; the verify function is the
guard that the ids still describe reality. The companion prose doc lives at
