...
- SAVE what re-verification needs: test outputs/fit objects/per-row predictions as artifacts (`save_artifacts`), so diagnosing a later failure does not force a re-run.
...
REF: `software-craft` (the build side) · `verification-loop` (the house marker/fixture pattern — verify_claims, both-way fixtures, verify-the-verifier) · efficacy is a MEASUREMENT, never an authoring-time assertion (a fix's existence is not evidence it works).
