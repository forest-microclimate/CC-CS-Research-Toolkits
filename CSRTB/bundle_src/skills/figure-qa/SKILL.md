...
RENDER THE REAL ARTIFACT AND OPEN IT. The saved file, at final size, actually looked at — not the plotting code, not a thumbnail, not the object printing without error. Numeric summaries and clean exits miss broken panels, empty facets, and visual pattern errors by construction (the recorded failures were all expert-caught BY EYE). Read the rendered image back into the session (display the PNG in a cell / rasterize the PDF page) and save the inspected render as an artifact — a save you never re-opened is unverified, and an un-saved figure is invisible next session.
