...
TIER_MODEL_IDS = frozenset(v["model"] for v in TIER_TABLE.values())
_TIER_MODEL_IDS_LC = frozenset(m.lower() for m in TIER_MODEL_IDS)
...
SELF_TOKENS = frozenset({"main-agent", "mainagent", "main", "self", "myself", "me",
...
MUTATION_RE = re.compile(
    ...
    r"append\w*|prun\w*|purg\w*|reset|rebuild\w*|regenerat\w*)\b", re.I)

ROUTING_FENCE_RE = re.compile(r"```+\s*routing\s*\r?\n(.*?)```+", re.S | re.I)
...
def _walk_strings(obj, path=""):
    ...
    if isinstance(obj, dict):
        for k in obj:
            ...
            for row in _walk_strings(obj[k], sub):
                yield row
    elif isinstance(obj, (list, tuple)):
        for i, v in enumerate(obj):
            for row in _walk_strings(v, "%s[%d]" % (path, i)):
                yield row
...
def _resolve_live_models(list_models):
...
def model_route_gate(route_map, list_models=None):
    ...
    if not isinstance(route_map, dict):
        try:
            ...
            return {"gate": "model_route_gate", "verdict": "FAIL", "n": 0,
                    ...
                    "vacuous": False,
                    "marker": _marker("route_gate", "FAIL", n=0)}

    live, live_err = _resolve_live_models(list_models)
    if live_err:
    ...
    for child in sorted(route_map, key=lambda c: str(c)):
        ...
        if banned:
            failures.append({"child": str(child), "model": mid, "check": "banned",
                             "detail": why})
            continue
        if mid.lower() in _TIER_MODEL_IDS_LC:
            continue
        if live is None:
    ...
    return {"gate": "model_route_gate", "verdict": verdict, "n": len(route_map),
            ...
            "marker": _marker("route_gate", verdict, n=len(route_map))}
...
def extract_routing_tracks(routing_block):
    ...
    if isinstance(routing_block, str):
        fences = ROUTING_FENCE_RE.findall(routing_block)
        if fences:
            tracks = []
...
def _is_commit_class(kind, topology, task_text):
    """L6 scope: delegate:* | parallel-wave | CODE:<cmd> whose task mutates."""
    if kind == "delegate":
        return True
    if topology == "parallel-wave":
        return True
    if kind == "CODE" and MUTATION_RE.search(task_text or ""):
        return True
    return False
...
def plan_lint(routing_block, solo=None):
    ...
    for idx, track in enumerate(tracks):
        ...
        for field in REQUIRED_ALWAYS:
            ...
            elif field != "recon_done" and _blank(track.get(field)):
                failures.append({"track": tid, "check": "L2", "field": field,
        ...
        topology = _norm(track.get("topology"))
        ...
        task_text = _norm(track.get("task"))
            ...
            if access and access != "self-service" and not access.startswith("precis-only:"):
                failures.append({"track": tid, "check": "L2", "field": "record_access",
                                 "detail": "must be 'self-service' or 'precis-only:<why>', "
                                           "got %r" % access})
            brief = _norm(track.get("brief_ref"))
        ...
        if topology and topology != "single-thread" and _blank(track.get("tradeoff")):
        ...
        if topology == "parallel-wave":
            conc = _norm(track.get("concurrency"))
        ...
        if kind == "delegate":
            if is_self_delegate(name):
                ...
                for field in SELF_RUN_SCAN_FIELDS:
                    text = _norm(track.get(field))
                    if not text:
                        continue
                    hit = next((r for r in SELF_RUN_TELLS if r.search(text)), None)
                    if hit is not None:
        ...
        # --- L4 tier ban -----------------------------------------------------
        if kind == "delegate":
            tier = _norm(track.get("model_tier"))
            if "model_tier" not in track or not tier:
                ...
                failures.append({"track": tid, "check": "L4", "field": "model_tier",
                                 "detail": "tier %r not in TIER_TABLE (%s)"
                                           % (tier, ", ".join(sorted(TIER_TABLE)))})
        for path, value in _walk_strings(track):
        ...
        # --- L5 reserved step -------------------------------------------------
        if track.get("reserved_for_user") is True and solo and MUTATION_RE.search(task_text):
        ...
        if _is_commit_class(kind, topology, task_text) and _blank(track.get("recon_done")):
    ...
    return {"gate": "plan_lint", "verdict": verdict, "tracks": len(tracks),
            ...
            "marker": _marker("plan_lint", verdict, tracks=len(tracks))}


# ─── GATE 3 — route_receipt_audit (post-wave) ────────────────────────────────
def _dispatched_name(row):
...
def route_receipt_audit(declared, dispatched):
    ...
    for idx, track in enumerate(tracks):
        ...
        declared_display.setdefault(key, _norm(name))
        declared_tracks.setdefault(key, []).append(tid)

    dispatched_names = []
    dispatched_display = {}
    for row in (dispatched or []):
        name = _dispatched_name(row)
        if not name:
    ...
    return {"gate": "route_receipt_audit", "verdict": verdict,
            ...
            "marker": _marker("route_audit", verdict, declared=len(declared_names),
