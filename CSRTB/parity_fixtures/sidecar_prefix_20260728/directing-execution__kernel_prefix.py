...
Host-independent, stdlib-only, Python 3.9+. Library use NEVER calls sys.exit; only the
argparse CLI (`python3 kernel.py < case.json`; exit 0 PASS / 2 FAIL) does.
"""
import argparse
import json
import os
...
NO_ARTIFACTS_RE = re.compile(r"^\s*child\s+produced\s+no\s+artifacts\s*:\s*(.+)$", re.I | re.S)

# Matched against the WHOLE normalized receipt, never as a substring -- so a real
# reference that merely contains the word "none" is not rejected.
...
_RECEIPTS = ("user_intent_ref", "artifacts_read")
...
def _flatten(value):
    ...
    if isinstance(value, (list, tuple)):
        return " ".join(p for p in (_flatten(v) for v in value) if p).strip()
...
def _is_placeholder(text):
    return _normalize(text) in PLACEHOLDER_TOKENS


def _split_ref_summary(text):
    ...
    if isinstance(entry, dict):
        ...
        for k in ("summary", "note", "read", "detail", "description", "contents"):
            if str(entry.get(k) or "").strip():
                ...
                break
        return ref, summary
    return _split_ref_summary(_flatten(entry))
...
def check_artifacts_read(value):
    ...
    raw = [e for e in raw if _flatten(e)]
    ...
    if declared_none:
        ...
        if _is_placeholder(how) or len(how) < MIN_SUMMARY_CHARS:
    ...
    for i, entry in enumerate(raw):
        ref, summary = _entry_ref_summary(entry)
        if not ref:
        ...
        if not summary:
            ...
            continue
        if len(summary) < MIN_SUMMARY_CHARS or _is_placeholder(summary):
            out["defects"].append("entry %d (%s): summary %r is too thin to be a read-receipt"
                                  % (i, ref, summary))
            continue
        if _normalize(summary) == _normalize(ref):
...
def confirm_before_stop(child, user_intent_ref=None, artifacts_read=None):
    ...
    child_txt = _flatten(child)
    if not child_txt or _is_placeholder(child_txt):
...
def _is_script_run():
    ...
    The documented load path is exec(host.skills.read("directing-execution",
...
if __name__ == "__main__" and _is_script_run():
    sys.exit(main())
