...
# STATUS: CURRENT. Mechanizes the Stale_Trash move-and-tombstone contract (see RULE.stale_trash in CLAUDE.md).
# CONTRACT: MOVE-AND-TOMBSTONE, never move-instead-of-tombstone.
...
# Pre-check: the ledger append is the LAST step, AFTER the irreversible tombstone+move.
...
#     Writability was pre-validated above; this is the TOCTOU belt — if the ledger became
