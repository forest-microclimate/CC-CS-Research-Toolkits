...
MODE.minimal-default: the installer has two modes, + the default is the MINIMAL one — it installs two files at your project root, the rules file (`CLAUDE.md`) + the folder contract (`STRUCTURE_RULES.machine.md`), plus the two advisory hooks (§7) + the model-routing set (two executor agents + a model-verification skill under `.claude/`), + creates NO folders; the project's tree grows on demand afterward, as tasks call for it.
...
FACT.two-files-enough: you do not have to hold any of this in memory. The folder contract, `STRUCTURE_RULES.machine.md`, is the authoritative description of every folder + the rule that governs it, + the rules file, `CLAUDE.md`, carries the disciplines those rules refer to. Between them, the two files the installer put at your root are enough to teach any fresh agent — + you — how the project is meant to work.
