...
- VALIDATE ON A MINIMAL INTERACTIVE STEP before any batch/full run; save outputs at the finest resolution.
- OPTIMIZE ONLY WHAT YOU MEASURED — profile first; language-specific performance/correctness traps live in `julia-performance-correctness` (Julia: BLAS threads, precompile races) and `rules/r-standards.md` (R: bam/gamm, silent-failure patterns) — reach for those, do not re-derive.
- ONE VARIABLE PER TEST CYCLE while iterating; keep scratch/diagnostics in a sandbox dir, never the working tree.

## Handoff
A substantial or high-consequence build ends with an adversarial review by a FRESH party (`code-review-debugger` agent, verify-loop topology) — the author's own green suite is not certification. State in the deliverable what was NOT verified.

REF: `testing-discipline` (the check contract) · `refactoring` (structural changes) · `julia-performance-correctness` · `rules/r-standards.md` · `rules/refactor-invariants.machine.md` · `rules/reproduce-before-fixing.machine.md` (the falsify-first sibling on the debug side).
