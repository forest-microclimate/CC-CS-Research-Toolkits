...
description: Invoke WHEN the task is to recover the IMPLICIT rationale, design philosophy, or governing principles behind a body of work — a codebase, toolkit, literature, method, dataset, or experimental program — and state them explicitly; OR to abstract a transferable schema from concrete instances and say what generalizes beyond them. This is the THINKING procedure that recovers, grounds, and grades rationale (inventory → cluster by friction → abstract → ground to instance → grade stated/inferred → tag scope → falsify on an out-of-corpus case). Fires on "what's the design philosophy here", "reverse-engineer the rationale", "what's the transferable principle", "what generalizes from these examples", "why is it built this way". NOT for revising existing prose (→ writing-science), setting an expert register (→ expert-prose-style), or writing the teaching guide itself (→ teaching-narrative) — those RENDER the content this procedure produces.
---
...
over the corpus. That failure is the whole risk of this task, and prose exhortations
...
The skill bundles `rationale_ledger.py` in its own directory — a traceability-ledger
validator that makes the Tier-1 disciplines *structural* instead of advisory. There is no
auto-load and no persistent kernel: build one row per principle in a JSON ledger, then run
the validator, which fail-closes on any row missing a discipline. One row, for the
installer-idempotency principle:
...
[
  {
    ...
    "friction_grade": "inferred",
    "falsification": "Checked against Homebrew formulae: idempotent re-run holds — fits."
  }
]
```

The grade choices carry the reasons: `grade="stated"` because the README states this aim;
`scope="schema"` because it is claimed for any installer, not just this one;
`friction_grade="inferred"` because the work does not say this, you read it from the design.
Validate the ledger, then render it — the script is pure `python3` stdlib, invoked directly
(pass `-` as the path to read the ledger from stdin):

```bash
# PASS/FAIL + each failing row's violated disciplines; exit 0 = all rows clean, 1 = any violation
python3 "$HOME/.claude/skills/design-rationale/rationale_ledger.py" validate ledger.json
# the grounded content as a markdown audit table, ready to hand to teaching-narrative or ship
python3 "$HOME/.claude/skills/design-rationale/rationale_ledger.py" render ledger.json
```

`validate` enforces: ≥1 instance (traceability), grade ∈ {stated, inferred} (grading),
scope ∈ {instance, schema} (scope-of-validity), a friction or a `value-driven:` note with a
graded friction (friction-grounding), and — for `schema` rows only — a non-empty
falsification result. An ungrounded principle cannot pass; the non-zero exit is a build gate
you can chain (`... validate ledger.json && <ship the writeup>`). The same validator
functions (`new_ledger`, `add_principle`, `validate_ledger`, `ledger_to_markdown`) are
importable from the script if you build the ledger programmatically instead.
