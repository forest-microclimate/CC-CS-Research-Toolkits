...
# writing_detectors.py — CC re-ship of the Claude Science writing-science kernel.py.
# STATUS: CURRENT (2026-07-14). Detector/utility function bodies are VERBATIM from the
# Science kernel; Claude Code has no auto-load and no persistent kernel, so the sole
# addition is the argparse `__main__` CLI at the bottom (run the script explicitly).
#   python3 writing_detectors.py scan draft.txt          # human triage report
#   python3 writing_detectors.py scan --json draft.txt   # structured JSON {counts,profile,hits}
#   (file arg '-' or omitted => read the draft from stdin)
...
def find_overcorrection(before_text, after_text):
    ...
    only surfaces suspects to judge against the TARGET reader). Invoked via `writing_detectors.py compare
    BEFORE AFTER`; it is NOT a per-draft detector and is deliberately absent from detector_map()."""
    def htoks(t):
...
# ----------------------------------------------------------------------------- CLI
# CC re-ship: no auto-load, no persistent kernel — run this script explicitly. Pure
# python3 stdlib (macOS 3.9.6 floor). `scan` prints report(); `scan --json` prints the
# scan_draft() dict as JSON (this is the input teaching-narrative's teaching_reframe.py reads).
if __name__ == "__main__":
    import argparse, sys, json as _json

    ap = argparse.ArgumentParser(
        prog="writing_detectors.py",
        description="writing-science mechanical draft detectors (Schimel). "
                    "Measure a draft's actual shape BEFORE judging it.")
    sub = ap.add_subparsers(dest="cmd", required=True)
    sp = sub.add_parser("scan", help="scan a draft and print the tell report")
    sp.add_argument("file", nargs="?", default="-",
                    help="draft text file; '-' or omitted reads stdin")
    sp.add_argument("--json", action="store_true",
                    help="emit the structured scan_draft() dict as JSON "
                         "{counts, profile, hits} instead of the human report")
    sp.add_argument("--top", type=int, default=5,
                    help="human report: top-N hits shown per fired detector (default 5)")
    # P2 item 3: BWT-007 over-correction, COMPARISON-ONLY (before vs after) — cautious tier.
    cp = sub.add_parser("compare", help="BWT-007 over-correction: surface hedges / field-standard terms "
                                        "a BEFORE->AFTER de-jargon pass may have wrongly stripped")
    cp.add_argument("before", help="the BEFORE draft text file ('-' reads stdin)")
    cp.add_argument("after", help="the AFTER draft text file")
    cp.add_argument("--json", action="store_true", help="emit the compare result as JSON")
    args = ap.parse_args()
    ...
    if args.cmd == "scan":
        _text = _read(args.file)
        if args.json:
            sys.stdout.write(_json.dumps(scan_draft(_text), ensure_ascii=False, indent=2))
            sys.stdout.write("\n")
        else:
            print(report(_text, top=args.top))
    elif args.cmd == "compare":
        _res = find_overcorrection(_read(args.before), _read(args.after))
        if args.json:
            sys.stdout.write(_json.dumps(_res, ensure_ascii=False, indent=2))
            sys.stdout.write("\n")
        else:
            print("# BWT-007 over-correction comparison (BEFORE -> AFTER) - CAUTIOUS candidates")
            print("\n## Stripped hedges (a legitimate qualification may have been cut)")
            for h in _res["stripped_hedges"] or [{"note": "(none)"}]:
                print("- " + h["note"])
            print("\n## Removed field-standard-looking terms (keep terms the domain reader expects)")
            for h in _res["removed_field_terms"] or [{"note": "(none)"}]:
                print("- " + h["note"])
