...
BUNDLED_TOOL: `sci_library_curate.py` (this dir). Pure python3 stdlib -- portable macOS+Linux, no third-party deps. 5 subcommands: the pipeline (`dedup` -> `migrate` -> `organize`) plus two integrity tools (`validate` = fail-loud structural invariants; `catalog` = regenerate the one-row-per-WORK clean lookup table). Run them, do NOT re-derive the logic by hand.
INVOKE: `python3 <skill_dir>/sci_library_curate.py <dedup|migrate|organize|catalog|validate> --index <paper_index.csv> [...]`.
Claude Code has no kernel auto-load and no in-kernel language-model call: run the tool from the shell, and where the pipeline needs a language model (the `organize` classification tail), YOU are the classifier -- see "## The organize model".
...
## Prerequisite
A finished `paper_index.csv` from sci-file-index whose filenames are in sync with disk. The Papers.app folder drifts continuously (it appends `_N`, swaps ` - supplement ` for `_supplement_`, etc.), so **re-extract the index immediately before curating** if any time has passed. The `migrate` subcommand is drift-resistant (it re-resolves each name to disk by exact-then-stem match) but will FATAL rather than guess if a keep-file has drifted beyond a stem match — that is the signal to re-extract.

## Environment
Pure python3 stdlib — the system `python3` works (validated on macOS 3.9.6). Nothing to install, no kernel to load. The library folders are local paths: the source (`papers library`) is read only; the clean destination is written to. Run every subcommand with the same interpreter.

## The three steps

```bash
# paths (edit to your library)
ROOT="/path/to/library"
IDX="$ROOT/index/paper_index.csv"
HASH="$ROOT/index/_sfi_filehashes.tsv"   # optional; size for cleanest-version tie-break
SRC="$ROOT/papers library"
DST="$ROOT/papers clean no dups"
SLC="python3 <skill_dir>/sci_library_curate.py"

# STEP 2 — dedup (read-only; writes decision + report, deletes nothing)
$SLC dedup --index "$IDX" --hashes "$HASH" --out "$DST/_dedup_decisions.csv" --report "$DST/DEDUP_REPORT.md"
...
#          supplement(s) is bundled into its own folder (article + its SIs). Add --no-bundle for flat.
$SLC migrate --index "$IDX" --decisions "$DST/_dedup_decisions.csv" --src "$SRC" --dst "$DST" --dry-run   # preview names + bundle counts
$SLC migrate --index "$IDX" --decisions "$DST/_dedup_decisions.csv" --src "$SRC" --dst "$DST"             # copy -> writes _MIGRATION_MANIFEST.csv (bundle_folder col)

# STEP 4 — organize into a Topic/Subtopic tree (two-pass: regex, then YOU classify the tail)
$SLC organize --index "$IDX" --manifest "$DST/_MIGRATION_MANIFEST.csv" --dst "$DST"   # 1st pass -> _unclassified_for_llm.json, stops
#   -> READ "$DST/_unclassified_for_llm.json" = {categories:[...], tasks:[{index_name,title,journal},...]}.
#      For each task pick the single best-matching string from categories; write {index_name: "Topic/Subtopic"}
#      to "$DST/_llm_assignments.json". (Supplements inherit their article's topic automatically.)
$SLC organize --index "$IDX" --manifest "$DST/_MIGRATION_MANIFEST.csv" --dst "$DST" \
      --llm-assignments "$DST/_llm_assignments.json"   # 2nd pass -> moves files, writes _LIBRARY_INDEX.csv

# VALIDATE (the vaccine) — run after ANY mutating stage (build, dedup, migrate, a manual edit).
# Exit 0 = all structural invariants pass; 1 = a FAIL was printed. With --lib it also checks
# disk<->index 1:1 and folder==article-stem. Catches drift (year-first names, duplicate clean_name,
# first_author_ascii out of sync, missing author/year/title) at the source.
$SLC validate --index "$IDX"                                             # index-only invariants
$SLC validate --index "$IDX" --lib "$DST" --report "$DST/VALIDATE_REPORT.md"   # + disk checks + report
...
# listed inline (n_supplements, component_paths, supplement_desc). It is a MATERIALIZED VIEW of the
# master: regenerate after ANY build/dedup/migrate/manual edit; NEVER hand-edit it. With --lib it
# re-runs the disk 1:1 reconciliation and exits 1 if the catalog would drift from disk (0 = clean).
$SLC catalog --index "$IDX" --out "$ROOT/_index_clean_lookup_table.csv" --lib "$DST"
```
...
## The organize model
- **Taxonomy** = `{Topic: {Subtopic: regex}}`. The built-in default (in `sci_library_curate.py`, `default_taxonomy()`) is plant-ecophysiology + microbial-ecology; **edit it, or pass a `--taxonomy` JSON, to match a different research domain.** The user's own framing should drive the top categories and subtopics.
- **Two-pass classification:** high-precision regex on `title + journal` handles the clear ~60%; the `organize` first pass writes the rest to `_unclassified_for_llm.json` and STOPS. YOU (the agent) are the second pass: read that file — `{categories:[...], tasks:[{index_name,title,journal},...]}` — pick the single best-matching category string for each task, and write `{index_name: "Topic/Subtopic"}` to `_llm_assignments.json`; re-run `organize --llm-assignments` to place everything. Supplements then inherit their parent article's topic. Files that still fall through land in an "Other / general ecology" catch-all.
...
- **Identity-integrity invariants I16–I19 (ported from the Claude Science sibling, 2026-07-24):** I16 (no unresolved supplement — every supplement/dataset resolves to a MAIN parent OR carries an `orphan_parent_absent` flag), I17 (no cryptic clean_name — no publisher-code/bare stem survives without a `cryptic_unresolved` flag; `is_cryptic_name` gate), I18 (SI/parent DOI agreement — a linked SI's own content DOI via `--lib` PDF masthead-mining, or its stored DOI, must match its parent's; repo-DOI and supplement-suffix tolerant), I19 (article identity real — an article row must have a non-blank title). These match the Claude Science curator's I16–I19 by logic (parallel implementation, not a byte-copy); brings CC-side parity with the Science validator.
