...
# STATUS: CURRENT (2026-08-09). GENERAL-PAYLOAD MODEL ROUTE, shipped from TWO homes byte-identically: `payload/agents/` (installed into the general `~/.claude` by `install.sh`) and `planner-kit/payload/.claude/agents/` (installed into each adopting project root by the kit's own installer). `lib/verify_models.sh` OWNS the contract that makes this legal — a NAMED allowlist of exactly three (fable-executor · fable-subplanner · opus5-executor), a MISSING pin on one of them is itself a FAIL, and the two homes must be byte-identical — so read the rule there rather than from a restatement here. [SUPERSEDED 2026-08-09, see `lib/verify_models.sh` contract 1 + its named ALLOWLIST] ~~Project-specialty carrier — installed per-project via `--project-items fable-executor` / `--project-bundle fable-executor`, never into the general ~/.claude payload.~~ — `payload-project/` was DELETED and those installer flags now exit 2 naming the manual-install `CCRT_specialists/` tree. WHAT DID NOT CHANGE: the blast-radius reasoning that barred the pin still governs every agent OUTSIDE the named three — a frontmatter pin is rank 3 of the measured precedence, so it overrides the launcher's tier choice in every session that installs it, and on an ordinary agent the pin IS the defect. What changed is the discriminator: for these three the pin IS the route (launching `delegate:fable-executor` is itself the tier choice, and the measured serving-substitution bug is why a route must place its request from frontmatter rather than from the launcher), so the exemption travels with the NAME, never with a directory. Built to `dev/reports/FABLE_EXECUTOR_SPEC.md` §2; efficacy `fixture-measured` — that spec's §6 five-launch acceptance PASSED 2026-08-04 16:56 (5/5 launches incl. a working run, every assistant turn stamped `claude-fable-5`; routing is what §6 measures — task efficacy is not yet measured). The `tools:` grant is LOAD-BEARING, not a convenience: `Skill` is the measured trigger of the open serving-side substitution bug (Read+Grep+Skill was served claude-opus-5 3/3, against Read+Grep served fable 2/2), so granting it would silently route this agent to another model; `Agent` is excluded by doctrine — an executor spawns nothing. Launch PARAMLESS so the rank-3 pin governs, and keep `CLAUDE_CODE_SUBAGENT_MODEL` at `inherit`/unset — rank 1 overrides both the param and the pin. EXTENDED 2026-08-06 (Z1, kit v1.7): VERIFIED-LAUNCH convention + tiered GRANT RATIONALE sections added below.
